  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }} className="space-y-4">
      ...
      <div>
        ...
        <select className="input" required value={form.instrument_id} onChange={(e) => set('instrument_id', e.target.value)}>
      ...
      <div>
        ...
        <select className="input" required value={form.default_storage_location_id} onChange={(e) => set('default_storage_location_id', e.target.value)}>
      ...
      <div>
        ...
        <input className="input font-mono" required value={form.cron_expression} onChange={(e) => set('cron_expression', e.target.value)} placeholder="0 * * * *" />
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label htmlFor="sched-enabled" className="text-sm font-medium text-gray-700">Enabled</label>
      </div>
...
export function Schedules() {
  ...
  const columns = [
    { header: 'Instrument', render: (row: HarvestSchedule) => instMap[row.instrument_id] ?? row.instrument_id.slice(0, 8) },
    { header: 'Storage', render: (row: HarvestSchedule) => storageMap[row.default_storage_location_id] ?? row.default_storage_location_id.slice(0, 8) },
    { header: 'Cron', render: (row: HarvestSchedule) => <code className="text-xs bg-gray-100 px-1 py-0.5 rounded">{row.cron_expression}</code> },
    {
      header: 'Prefect',
    ...
    },
    {
      ...
      render: (row: HarvestSchedule) => (
        <div className="flex gap-2">
          <button className="btn btn-sm btn-secondary" onClick={() => setModal({ kind: 'edit', schedule: row })}>
            Edit
          </button>
          <button
            className="btn btn-sm btn-danger"
            onClick={() => { if (confirm('Delete this schedule?')) del.mutate(row.id) }}
          >
            Delete
          </button>
        </div>
  ...
  return (
    <div>
      ...
      <div className="card p-0 overflow-hidden">
        <Table columns={columns} data={schedules} isLoading={isLoading} emptyMessage="No schedules configured." />
      </div>
      ...
      {modal.kind === 'edit' && (
        <Modal title="Edit Harvest Schedule" onClose={close}>
          <ScheduleForm
            ...
            onSubmit={(data) => update.mutate({ id: modal.schedule.id, data }, { onSuccess: close })}
            onCancel={close}
